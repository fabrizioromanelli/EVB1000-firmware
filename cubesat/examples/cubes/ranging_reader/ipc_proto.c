...
int get_data_vectors(ipc_channel_t * c, uint16_t * adjvec, coordinates_t * coord_vector, uint64_t * timestamp) 
{
	...
	int adjsize = num_of_nodes * (num_of_nodes - 1) / 2;
	memcpy(adjvec, readbuffer, adjsize * sizeof(uint16_t));
	memcpy(coord_vector, 	readbuffer + adjsize, num_of_nodes * sizeof(coordinates_t));
	memcpy(timestamp, 	readbuffer + adjsize + num_of_nodes * sizeof(coordinates_t), sizeof(uint64_t));
...
int write_coordinates(ipc_channel_t * c, double lat, double lon)
{
	...
	int offset = num_of_nodes * sizeof(uint16_t) + nodeindex * sizeof(coordinates_t);
