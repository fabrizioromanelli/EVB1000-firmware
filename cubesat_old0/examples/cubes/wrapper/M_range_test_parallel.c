...
static void dist(nbr_t node, int status) {
	...
	fflush(stdout);
	if(status) {
		printf("\r[C][NODE %X] distance: %.2f m", node.id, node.distance/100.0);
		fflush(stdout);
	}
	else{
		printf("\r[C][NODE %X] Rng error, status %d", node.id, status);
		fflush(stdout);
	}
	sleep(5);
	fprintf(stdout, "\n[C]end callback\n");
...
int main(int argc, char *argv[]) {
	...
	while(1) {
		...
		if(!status){
			printf("[M][NODE %X] ERROR! cannot perform range test\r", my_id);
			fflush(stdout);
		}
		fprintf(stderr, "\n[M]ranging terminated\n");
		fflush(stdout);
		sleep(1);
	}
