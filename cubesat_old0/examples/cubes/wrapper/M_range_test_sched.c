...
#define print(str, node_id, node_time, ...) struct timespec meas;\
				...
				printf("\r[NODE %X - %.2f s] " str, node_id, timespec_to_sec(meas, start_time), __VA_ARGS__);\
				fflush(stdout);
...
int * static_node_list = {NODE_1, NODE_2}
...
int perform_ranging_procedure(struct timespec timeout_counter) {
		...
		if(rng_c_stat == RANGE_COMPLETED || timespec_to_sec(timeout_temp, timeout_counter) >= RANGING_TIMEOUT) {
			...
			if(!status){
				struct timespec meas;
				clock_gettime(CLOCK_MONOTONIC_RAW, &meas);
				printf("\r[NODE %X  - %.2f s] ERROR! cannot perform range test       ", my_id, timespec_to_sec(meas, start));
				fflush(stdout);
		...
		// the writing of the distance variable is asynchronous
			...
			struct timespec meas;
			clock_gettime(CLOCK_MONOTONIC_RAW, &meas);
			printf("\r[NODE %X  - %.2f s] distance: %.2f m, required time: %.2f ms ", my_id, timespec_to_sec(meas, start), distance, timespec_to_sec(meas, timeout_counter) * 1e3);
			fflush(stdout);
			...
			struct timespec meas;
			clock_gettime(CLOCK_MONOTONIC_RAW, &meas);
			printf("\r[NODE %X  - %.2f s] Rng error, collision                     ",  my_id, timespec_to_sec(meas, start));
			fflush(stdout);
			rng_c_stat = RANGE_COMPLETED;
