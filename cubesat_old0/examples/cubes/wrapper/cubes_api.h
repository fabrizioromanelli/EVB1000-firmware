...
#define MAX_PAYLOAD_LEN 64 ///< The maximum app payload length
...
/**
 * Flush the output packet queue. 
 *
 * Note that the send callbacks might not be called for the
 * flushed packets.
 */
 ...
 *  over the table.
 */
int nbr_get_from_nodeid(node_id_t node_id, nbr_t* nbr);
 ...
 *
 * @param descr A pointer to memory where the descriptor object of the last 
 * built adjacency matrix will be copied. Array pointers in the descriptor are 
 * guaranteed to be valid until the next call to this function.
 *
 * @return Zero if no adjacency matrix was ever built.
 */
int get_last_adjm(adjm_descr_t *descr);
...
/** Configuration structure (TBD)
 */
typedef struct {
  uint16_t beacon_interval; ///< Neighbor discovery beacon interval in seconds
  uint16_t nbr_timeout;     ///< Neighbor table entry expiration time in seconds
} config_t;

/** Get the current configuration. Useful also to get the default configuration
 *  on boot. 
 */
config_t get_config();
 ...
 *
 *  @return Zero if some configuration values are out of range
 */
